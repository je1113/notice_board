...
<%
	...
	//String userid = (String)session.getAttribute("idx");  idx를 사용하여 검색하면 속도가 유리
...
<head>
    ...
    <script>//daum 우편번호 api 검색 사용자가 선택한 값 이용하기내용
...
<body>
    <h2>정보수정</h2>
    <form action="InfoOk" name="regform" id="regform" method="post" onsubmit="return sendit()">
        <p>아이디: <%=userid %></p>
        ...
        <p>취미: 
            ...
            <% if(compareHobby(hobbyArr,"운동")) out.print("checked");%>></label>
        </p>
        <p>주민등록번호: <input type="text" name="ssn1" value="<%=mem_ssn1%>"> - <input type="text" name="ssn2" value="<%=mem_ssn2%>"></p>
        <p>우편번호: <input type="text" name="zipcode" maxlength="5" id="sample6_postcode" value="<%=mem_zipcode%>"> <button type="button" onclick="sample6_execDaumPostcode()" value="우편번호 찾기">검색</button></p>
